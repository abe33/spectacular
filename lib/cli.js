...
var Q, colorize, exists, fs, getOriginalSourceFor, getReporter, glob, globPath, globPaths, handleEmitter, jsdom, loadDOM, loadFile, loadHelpers, loadMatchers, loadSpecs, loadSpectacular, m, path, requireFile, sourceMap, util, vm, walk;
...
loadFile = function(options) {
  ...
  return function(file) {
    return Q.fcall(function() {
      ...
      if (options.coffee && /\.coffee$/.test(file)) {
      ...
      return cache[file] = fileContent || fileSource;
    });
  };
};

getOriginalSourceFor = function(options) {
  var cache;
  cache = {};
  return function(file, line, column) {
    var defer;
    defer = Q.defer();
    options.loadFile(file).then(function(compiled) {
      var consumer, _ref;
