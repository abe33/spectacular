...
spectacular.StackReporter = (function() {
  ...
  StackReporter.prototype.report = function() {
    var c, column, done, e, line, match, pre, stack, url, _ref,
      _this = this;
    ...
    _ref = /(http:\/\/.*\.(js|coffee)):(\d+)(:(\d+))*/g.exec(line), match = _ref[0], url = _ref[1], e = _ref[2], line = _ref[3], c = _ref[4], column = _ref[5];
      ...
      return $("#pre_" + _this.id).html(_this.getLines(data, line, column)).removeClass('loading');
    };
    $.ajax({
      url: url,
  ...
  StackReporter.prototype.insertColumnLine = function(content, line, column) {
    ...
    if (line === content.length) {
      ...
      return content.splice(line, 0, "      |" + (this.padRight('^', column)));
    }
  };

  StackReporter.prototype.padRight = function(string, pad) {
    if (pad == null) {
      pad = 4;
    }
...
spectacular.BrowserReporter = (function() {
  ...
  BrowserReporter.prototype.printResults = function(lstart, lend, sstart, send) {
    ...
    return this.counters.find('#counters').append(", finished in " + (this.formatDuration(sstart, send)));
  };
  ...
  BrowserReporter.prototype.registerResult = function(example) {
    ...
    ex.click(function() {
      return ex.toggleClass('open');
    });
    return this.examplesContainer.append(ex);
  };
  ...
  BrowserReporter.prototype.traceSource = function(error) {
    return (new spectacular.StackReporter(error)).report();
  };
