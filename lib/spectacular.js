...
var GeckoMatch, PROGRESS_CHAR_MAP, PROGRESS_COLOR_MAP, URI_RE, V8Match, exports, isCommonJS, k, spectacular, utils, v,
  __slice = [].slice,
...
GeckoMatch = (function() {
  ...
  GeckoMatch.prototype.match = function(stack) {
    return /@/g.test(stack);
  };

  GeckoMatch.prototype.filter = function(line) {
    return /@/.test(line);
  };

  GeckoMatch.prototype.details = function(line) {
    var c, column, e, file, h, match, method, p, q, re, _ref;
    re = RegExp("\\s*([^\\s]*)(@)" + URI_RE + ":(\\d+)(:(\\d+))*");
...
V8Match = (function() {
  ...
  V8Match.prototype.match = function(stack) {
    return /\s+at\s+/g.test(stack);
  };

  V8Match.prototype.filter = function(line) {
    return /\s+at\s+/.test(line);
  };

  V8Match.prototype.details = function(line) {
    var c, column, e, file, h, match, method, p, q, re, _ref;
    re = RegExp("(at\\s*([^\\s]*)\\s*\\(|\\()" + URI_RE + ":(\\d+)(:(\\d+))*");
...
spectacular.ErrorParser = (function() {
  ErrorParser.supportedFormats = [new V8Match, new GeckoMatch];
...
spectacular.StackReporter = (function() {
  ...
  StackReporter.prototype.formatStack = function(stack) {
    ...
    if (this.options.longTrace) {
      s = "\n\n" + (stack.join('\n')) + "\n";
      if (this.isGeckoLikeStackTraceLine(s)) {
  ...
  StackReporter.prototype.isGeckoLikeStackTraceLine = function(line) {
    return /@/.test(line);
  };

  StackReporter.prototype.getLineDetails = function(line) {
    var c, column, e, file, h, match, p, re, res;
    if (this.isGeckoLikeStackTraceLine(line)) {
    ...
    match = res[0], p = res[1], file = res[2], h = res[3], e = res[4], line = res[5], c = res[6], column = res[7];
    return {
      file: file,
      line: line,
      column: column
  ...
  StackReporter.prototype.formatErrorInFile = function(line) {
    var column, details, file, promise;
    promise = new spectacular.Promise;
    details = this.getLineDetails(line);
    if (details == null) {
      promise.resolve('');
      return promise;
    }
    file = details.file, line = details.line, column = details.column;
    if ((column == null) && (this.error.columnNumber != null)) {
      column = this.error.columnNumber;
    }
    this.getLines(file, line, column).then(function(lines) {
  ...
  StackReporter.prototype.getLines = function(file, line, column) {
    var continuation, promise,
      _this = this;
    promise = new spectacular.Promise;
    continuation = function(fileContent) {
    ...
    if (this.options.coffee && this.options.sourceMap && this.options.hasSourceMap(file)) {
      this.options.getOriginalSourceFor(file, line, column).then(function(res) {
        var content;
        content = res.content, line = res.line, column = res.column;
        return continuation(content);
      });
    } else {
      this.options.loadFile(file).then(continuation);
    }
    return promise;
  };
  ...
  StackReporter.prototype.insertColumnLine = function(content, line, column) {
    ...
    if (line === content.length) {
      ...
      return content.splice(line, 0, "         | " + (utils.padRight('^', column + 1)));
    }
  };
