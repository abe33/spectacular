...
spectacular.Globalizable = (function() {
  ...
  Globalizable.prototype.globalize = function() {
    ...
    this.EXPOSED_PROPERTIES.forEach(function(k) {
      var proxy, self, value;
      ...
      if (typeof value === 'function') {
        value._name = k;
        if (_this.keepContext) {
          proxy = function() {
            return value.apply(self, arguments);
          };
          proxy._name = k;
          return _global[k] = proxy;
        } else {
          return _global[k] = value;
        }
      } else {
        return _global[k] = value;
      }
...
spectacular.GlobalizableObject = (function() {
  ...
  function GlobalizableObject() {
    var __EXPOSED_PROPERTIES;

    __EXPOSED_PROPERTIES = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    this.__EXPOSED_PROPERTIES = __EXPOSED_PROPERTIES;
  }

  Object.defineProperty(GlobalizableObject.prototype, 'EXPOSED_PROPERTIES', {
    configurable: true,
    get: function() {
      if (this.__EXPOSED_PROPERTIES.length > 0) {
        return this.__EXPOSED_PROPERTIES;
      } else {
...
spectacular.factories.Trait = (function() {
  ...
  Trait.prototype.EXPOSED_PROPERTIES = 'set createWith'.split(/\s+/g);
...
spectacular.factories.Factory = (function(_super) {
  ...
  Factory.prototype.EXPOSED_PROPERTIES = 'set trait createWith'.split(/\s+/g);
...
spectacular.dom.NodeExpression = (function() {
  ...
  NodeExpression.prototype.match = function(el) {
    return el.is(this.expression) && this.expressions.every(function(e) {
  ...
  NodeExpression.prototype.handleTextExpression = function(el) {
    ...
    if (this.expression.indexOf('/') === 0) {
      return new RegExp(content).test(el.text());
    } else {
...
spectacular.Runner = (function() {
  ...
  Runner.prototype.registerSpecs = function() {
    var example, _i, _j, _len, _len1, _ref, _ref1, _results, _results1;

    if (this.root.hasExclusiveExamples()) {
      _ref = this.root.allExclusiveExamples;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        example = _ref[_i];
      ...
      _ref1 = this.root.allExamples;
      _results1 = [];
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        example = _ref1[_j];
        _results1.push(this.register(example));
      }
      return _results1;
    }
  };
...
spectacular.Environment = (function() {
  ...
  Environment.prototype.EXPOSED_PROPERTIES = 'it xit describe xdescribe context xcontext\
  ...
  Environment.prototype.loadJQuery = function() {
  ...
  Environment.prototype.itsInstance = function(property, block) {
    var parentSubjectBlock, _ref,
      _this = this;

    this.notInsideIt('itsInstance');
    if (typeof property === 'function') {
      _ref = [block, property], property = _ref[0], block = _ref[1];
    }
    parentSubjectBlock = this.currentExampleGroup.subjectBlock;
    return this.context('instance', function() {
      _this.subject('instance', function() {
        return build(parentSubjectBlock != null ? parentSubjectBlock.call(this) : void 0, this.parameters || []);
      });
      if (property != null) {
  ...
  Environment.prototype.itsReturn = function(options, block) {
    ...
    return this.context('returned value', function() {
      _this.subject('returnedValue', function() {
        return parentSubjectBlock != null ? parentSubjectBlock.call(this).apply(options.inContext || this, options["with"] || this.parameters || []) : void 0;
      });
      return _this.it(block);
    });
  };
...
spectacular.matchers.have.selector = function(selector) {
  return {
    assert: function(actual, notText) {
      ...
      this.message = "Expected '" + (actual.html()) + "'" + notText + " to have selector '" + selector + "'";
      return actual.find(selector).length > 0;
    }
  };
};
  ...
  return {
    assert: function(actual, notText) {
      ...
      if ((re.match != null) && (re.contained != null)) {
        this.message = "Expected '" + ($.map(actual, function(e) {
          return e.outerHTML;
        })) + "'" + notText + " to match " + re;
