...
var exports, isCommonJS, k, nextTick, spectacular, utils, v, _ref,
  __slice = [].slice,
...
spectacular.utils.stringDiff = function(o, n) {
  ...
  if ((n == null) || n.length === 0) {
    return utils.TAGS.delStart + o + utils.TAGS.delEnd;
  }
  if ((o == null) || o.length === 0) {
    return utils.TAGS.insStart + n + utils.TAGS.insEnd;
  }
...
spectacular.utils.objectDiff = function(left, right, depth) {
  ...
  if (typeLeft !== typeRight) {
    s = '';
    if (left != null) {
      s += utils.TAGS.delStart + utils.inspect(left, depth) + utils.TAGS.delEnd;
    }
    if (right != null) {
      s += utils.TAGS.insStart + utils.inspect(right, depth) + utils.TAGS.insEnd;
    }
    return s;
  }
  switch (typeLeft) {
    ...
    case 'object':
      if (utils.isArray(left) !== utils.isArray(right)) {
        return utils.TAGS.delStart + utils.inspect(left, depth) + utils.TAGS.delEnd + utils.TAGS.insStart + utils.inspect(right, depth) + utils.TAGS.insEnd;
      }
      ind = utils.fill(depth * 2);
        ...
        case 'string':
          ...
          if (state != null) {
            if (typeof actual[state] === 'function') {
              ...
              this.message = "Expected " + actual + "." + state + "()" + notText + " to be true " + (r ? 'and' : 'but') + " was " + (actual[state]());
            } else {
              result = actual[state];
              r = notText.length === 0 ? result : !result;
              this.message = "Expected " + actual + "." + state + notText + " to be true " + (r ? 'and' : 'but') + " was " + actual[state];
...
spectacular.StackFormatter = (function() {
  ...
  StackFormatter.prototype.formatErrorInFile = function(line) {
    var c, column, e, file, h, match, p, promise, re, _ref1;
    ...
    _ref1 = re.exec(line), match = _ref1[0], p = _ref1[1], file = _ref1[2], h = _ref1[3], e = _ref1[4], line = _ref1[5], c = _ref1[6], column = _ref1[7];
...
spectacular.ConsoleReporter = (function() {
  ...
  ConsoleReporter.prototype.hasFailures = function() {
    return this.results.some(function(result) {
      ...
      return (_ref1 = result.state) === 'failure' || _ref1 === 'skipped' || _ref1 === 'errored';
    });
  };
  ...
  ConsoleReporter.prototype.formatResult = function(result) {
    var expectation, promise, _i, _len, _ref1;
