...
var PROGRESS_CHAR_MAP, PROGRESS_COLOR_MAP, exports, isCommonJS, k, nextTick, spectacular, utils, v,
  __slice = [].slice,
...
spectacular.Globalizable = (function() {
  ...
  Globalizable.prototype.globalizeMember = function(key, value) {
    var proxy, self, snake, _global;
    ...
    if (snake !== key) {
      return this.globalizeMember(snake, value);
    }
  };

  Globalizable.prototype.unglobalizeMember = function(key, value) {
    var snake, _global;
    _global = spectacular.global;
    if (this.previous[k] != null) {
      _global[k] = this.previous[k];
    } else {
      delete _global[k];
    }
    snake = utils.underscore(key);
    if (snake !== key) {
      return this.unglobalizeMember(snake, value);
    }
  };
...
spectacular.factories || (spectacular.factories = new spectacular.GlobalizableObject('build', 'create', 'factory'));
...
spectacular.factories.Trait = (function() {
  ...
  Trait.prototype.globalizable = 'set createWith'.split(/\s+/g);
...
spectacular.factories.Factory = (function(_super) {
  ...
  Factory.prototype.globalizable = 'set trait createWith'.split(/\s+/g);
...
spectacular.Expectation = (function() {
  ...
  Expectation.prototype.createMessage = function() {
    ...
    this.description = "" + this.ownDescription + this.matcher.description;
    return this.fullDescription = "" + this.example.description + " " + this.ownDescription + this.matcher.description;
  };
...
spectacular.Example = (function() {
  ...
  Example.include(spectacular.FollowUpProperty('inclusive'));

  Example.include(spectacular.FollowUpProperty('exclusive'));
    ...
    switch (typeof desc) {
      case 'string':
        if (desc.indexOf('.') === 0) {
          ...
          if ((this.parent == null) || this.parent.description === '') {
            this.noSpaceBeforeDescription = true;
          }
        }
        break;
...
spectacular.Environment = (function() {
  ...
  _globalize = Environment.prototype.globalize;

  _unglobalize = Environment.prototype.unglobalize;
  ...
  Environment.prototype.globalize = function() {
    ...
    this.globalizeObjectExtensions();
    return this.globalizeJQuery();
  };

  Environment.prototype.unglobalize = function() {
    _unglobalize.call(this);
    spectacular.factories.unglobalize();
    return spectacular.matchers.unglobalize();
  };

  Environment.prototype.globalizeJQuery = function() {
    return spectacular.global.$ = this.options.jQuery;
  };
  ...
  Environment.prototype.itsReturn = function(options, block) {
    ...
    return this.context('returned value', function() {
      _this.subject('returnedValue', function() {
        var context, params;
        context = options.inContext != null ? typeof options.inContext === 'function' ? options.inContext.call(this) : options.inContext : this;
        params = options["with"] != null ? typeof options["with"] === 'function' ? options["with"].call(this) : options["with"] : this.parameters || [];
  ...
  Environment.prototype.itBehaveLike = function(name, options) {
    if (options == null) {
      options = {};
    }
    if (!(name in this.sharedExamples)) {
...
spectacular.matchers.have.selector = function(selector) {
  return {
    assert: function(actual, notText) {
      var actualDesc;
      this.description = "should" + notText + " have content that match '" + selector + "'";
      if (actual.length != null) {
        actualDesc = Array.prototype.map.call(actual, function(e) {
          return e.outerHTML;
        });
        this.message = "Expected " + (utils.inspect(actualDesc)) + notText + " to have selector '" + selector + "'";
        ...
        this.message = "Expected " + (utils.inspect(actualDesc)) + notText + " to have selector '" + selector + "'";
        return actual.querySelectorAll(selector).length > 0;
      }
    }
  };
};

spectacular.matchers.be = function(desc, value) {
  if (value == null) {
    value = desc;
  }
  return {
    assert: function(actual, notText) {
      var r, result, state;
      this.description = "should" + notText + " be " + desc;
      switch (typeof value) {
        case 'string':
        ...
        case 'boolean':
          ...
          return (actual != null ? actual.valueOf() : void 0) === value;
        default:
          desc = typeof desc === 'string' ? desc : utils.squeeze(utils.inspect(value));
          this.description = "should" + notText + " be " + desc;
...
spectacular.matchers.match = function(re) {
  return {
    assert: function(actual, notText) {
      ...
      if ((re.match != null) && (re.contained != null)) {
        actualDesc = actual.length ? Array.prototype.map.call(actual, function(e) {
          return e.outerHTML;
        }) : actual.outerHTML;
        this.message = "Expected " + (utils.inspect(actualDesc)) + notText + " to match " + re;
...
spectacular.matchers.contains = function() {
  var value, values;
  values = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
  value = values[0];
  return {
    assert: function(actual, notText) {
      var actualDesc, valuesDescription;
      if ((value.match != null) && (value.contained != null)) {
        actualDesc = actual.length ? Array.prototype.map.call(actual, function(e) {
          return e.outerHTML;
        }) : actual.outerHTML;
        this.description = "should" + notText + " contains " + value;
        this.message = "Expected " + (utils.inspect(actualDesc)) + notText + " to contains " + value;
        return value.contained(actual);
      } else {
        valuesDescription = utils.literalEnumeration(values.map(function(v) {
          return utils.inspect(v);
        }));
        this.description = "should" + notText + " contains " + valuesDescription;
        this.message = "Expected " + (utils.inspect(actual)) + notText + " to contains " + valuesDescription;
