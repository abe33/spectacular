...
var exports, isCommonJS, nextTick, spectacular, u, utils,
  __slice = [].slice,
...
u = spectacular.utils || (spectacular.utils = {});
...
spectacular.utils.stringDiff = function(o, n) {
  ...
  if ((n == null) || n.length === 0) {
    return u.TAGS.delStart + o + u.TAGS.delEnd;
  }
  if ((o == null) || o.length === 0) {
    return u.TAGS.insStart + n + u.TAGS.insEnd;
  }
  o = o.replace(/\s+$/, "");
  n = n.replace(/\s+$/, "");
  out = u.diff((o === "" ? [] : o.split(/\s+/)), (n === "" ? [] : n.split(/\s+/)));
  str = "";
  oSpace = o.match(/\s+/g);
  if (oSpace == null) {
    oSpace = [u.TAGS.space];
  ...
  switch (typeof obj) {
    ...
    case 'object':
      ...
      if (u.isArray(obj)) {
        ...
        return "[\n" + (obj.map(function(o) {
          return ind + u.inspect(o, depth + 1);
        }).join(',\n')) + "\n" + ind.slice(0, -2) + "]";
      } else {
        if (u.keys(obj).length === 0) {
          return '{}';
        }
        return "{\n" + (((function() {
          var _results;
          ...
          for (k in obj) {
            v = obj[k];
            _results.push("" + ind + k + ": " + (u.inspect(v, depth + 1)));
          }
          return _results;
        })()).join(',\n')) + "\n" + ind.slice(0, -2) + "}";
      }
...
spectacular.utils.objectDiff = function(left, right, depth) {
  ...
  if (typeLeft !== typeRight) {
    s = '';
    if (left != null) {
      s += u.TAGS.delStart + u.inspect(left, depth) + u.TAGS.delEnd;
    }
    if (right != null) {
      s += u.TAGS.insStart + u.inspect(right, depth) + u.TAGS.insEnd;
    }
    return s;
  }
  switch (typeLeft) {
    case 'string':
      return u.inspect(u.stringDiff(left, right));
    case 'number':
    case 'boolean':
      return u.stringDiff(left.toString(), right.toString());
    case 'object':
      if (u.isArray(left) !== u.isArray(right)) {
        return u.TAGS.delStart + u.inspect(left, depth) + u.TAGS.delEnd + u.TAGS.insStart + u.inspect(right, depth) + u.TAGS.insEnd;
      }
      ind = u.fill(depth * 2);
    ...
    case 'object':
      if (u.isArray(actual)) {
        ...
        for (i = _i = 0, _len = value.length; _i < _len; i = ++_i) {
          v = value[i];
          if (!u.compare(actual[i], v, matcher, true)) {
            return false;
          }
        }
        return true;
        ...
        if (u.keys(actual).length !== u.keys(value).length) {
          return false;
        }
        for (k in value) {
          v = value[k];
          if (!u.compare(actual[k], v, matcher, true)) {
            return false;
          }
        }
        return true;
...
spectacular.StackFormatter = (function() {
  ...
  StackFormatter.prototype.formatErrorInFile = function(line) {
    var c, column, e, file, match, promise, re, _ref;
    ...
    _ref = re.exec(line), match = _ref[0], file = _ref[1], e = _ref[2], line = _ref[3], c = _ref[4], column = _ref[5];
...
spectacular.ConsoleReporter = (function() {
  ...
  ConsoleReporter.prototype.formatStack = function(e) {
    return new spectacular.StackFormatter(e, this.options).format();
  };
