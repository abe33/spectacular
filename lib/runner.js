...
var Q, Runner, fs, glob, nextTick, path, util,
...
Runner = (function() {
  ...
  Runner.prototype.printErrorInFile = function(line) {
  ...
  Runner.prototype.getLines = function(file, line, column) {
  ...
  Runner.prototype.insertColumnLine = function(content, line, column) {
    if (line === content.length) {
      return content.push(line);
    } else {
      return content.splice(line, 0, "         |" + (this.padRight('^', column - 2)));
    }
  };

  Runner.prototype.padRight = function(string, pad) {
    if (pad == null) {
      pad = 4;
    }
  ...
  Runner.prototype.printFailure = function(message) {
    ...
    badge = ' FAIL ';
    return console.log(this.options.noColors ? "" + badge + " " + message : ("" + badge.inverse.bold + " " + message).red);
  };

  Runner.prototype.printError = function(message) {
    var badge;

    badge = ' ERROR ';
    return console.log(this.options.noColors ? "" + badge + " " + message : ("" + badge.inverse.bold + " " + message).yellow);
  };
  ...
  Runner.prototype.printResults = function() {
    ...
    if (this.hasFailures()) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        switch (result.state) {
          case 'errored':
            this.printError(result.example.description);
  ...
  Runner.prototype.formatCounters = function() {
    var assertions, errored, failures, loadDuration, pending, skipped, specsDuration, success;
    ...
    failures = this.examples.filter(function(e) {
      ...
      return (_ref = e.result.state) === 'failure';
    }).length;
    errored = this.examples.filter(function(e) {
      var _ref;

      return (_ref = e.result.state) === 'errored';
    }).length;
